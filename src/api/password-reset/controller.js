...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your my-web-server account.<br>
        ...
        &mdash; my-web-server Team
      `
      return sendMail({ toEmail: email, subject: 'my-web-server - Password Reset', content })
    })
